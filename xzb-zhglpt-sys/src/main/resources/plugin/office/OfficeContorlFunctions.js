...
function intializePage(id, user) {
    //alert(fileUrl);
...
function NTKO_OCX_OpenDoc(id) {
    OFFICE_CONTROL_OBJ.BeginOpenFromURL("open?id=" + id);
}
...
function saveFileToUrl(date) {
    ...
    //OFFICE_CONTROL_OBJ.SaveToLocal("saveDoc?" + fileName, true);
    var result;
    if (IsFileOpened) {
        ...
        result = OFFICE_CONTROL_OBJ.saveToURL(myUrl,//提交到的url地址
            ...
            date + "/" + fileName,    //上传文件的名称，类似<input type=file 的value
            0    //与控件一起提交的表单id，也可以是form的序列号，这里应该是0.
        );
        console.log(result);
        result = JSON.parse(result);
        //result = trim(result);
        document.all("statusBar").innerHTML = "服务器返回信息:" + result.msg;
        //	alert(result);
    }
}
...
function insertRedHeadFromUrl(headFileURL, _num) {

    //alert(OFFICE_CONTROL_OBJ.doctype);
    ...
    num = _num;
    setTimeout("addWb()", 5000);

}

function addWb() {
    //alert(num);
    OFFICE_CONTROL_OBJ.ActiveDocument.BookMarks("编号").Range.Text = num;
}

//插入文尾
function insertButtomFromUrl(buttomFileURL) {
    //alert("文档套红");
    if (OFFICE_CONTROL_OBJ.doctype != 1)//OFFICE_CONTROL_OBJ.doctype=1为word文档
    {
        return;
    }
    OFFICE_CONTROL_OBJ.ActiveDocument.Application.Selection.EndKey(6);//光标移动到文档尾部
    OFFICE_CONTROL_OBJ.AddTemplateFromURL(buttomFileURL);//在光标位置插入红头文档
}
