...
<head>
    ...
    <meta name="viewport"
          content="width=device-width,notice-scalable=yes, minimum-scale=0.4, initial-scale=0.8,target-densitydpi=low-dpi"/>
...
<div class="layui-col-md12" style="height:40px;margin-top:3px;">
    <div class="layui-btn-group">
        ...
        <#--</@shiro.hasPermission>-->
        <#--<@shiro.hasPermission name="notice:del">-->
            <button class="layui-btn layui-btn-normal" data-type="export">
                <i class="layui-icon">&#xe605;</i>导出
            </button>
        <#--</@shiro.hasPermission>-->
        <#--<@shiro.hasPermission name="notice:repass">-->
...
<table id="noticeList" class="layui-hide" lay-filter="notice"></table>
    ...
    layui.use('table', function () {
        ...
        table.render({
            ...
            , cols: [[
                ...
                , {field: 'notice_name', title: '更新人', width: '20%', sort: true}
                , {field: 'create_time', title: '创建时间', width: '10%', sort: true}
                , {field: 'update_time', title: '更新时间', width: '10%', sort: true}
        ...
        var $ = layui.$, active = {
            ...
            add: function () {
                add('添加用户', 'showAddNotice',700, 680);
            },
            update: function () {
                ...
                update('编辑用户', 'updateNotice?id=' + data[0].id, 700, 450);
            },
            detail: function () {
                var checkStatus = table.checkStatus('noticeList')
                    , data = checkStatus.data;
                ...
                detail('查看用户信息', 'updateNotice?id=' + data[0].id, 700, 450);
            },
            /*changePwd:function(){
              var checkStatus = table.checkStatus('noticeList')
                  , data = checkStatus.data;
            ...
            changePwd: function () {
                ...
                $.ajax({
                    ...
                    success: function (json) {
                        ...
                        window.parent.layui.table.reload('noticeList');*/
        ...
        table.on('tool(notice)', function (obj) {
            var data = obj.data;
            if (obj.event === 'detail') {
                detail('查看用户', 'updateNotice?id=' + data.id, 700, 600);
            } else if (obj.event === 'del') {
                layer.confirm('确定删除', {
                    btn: ['逻辑删除', '物理删除']
                }, function (index) {
                    layer.close(index);
                    toolDelByFlag(data.id, '', false, 'noticeList');
                }, function (index) {
                    layer.close(index);
                    toolDelByFlag(data.id, '', true, 'noticeList');
                });
            } else if (obj.event === 'edit') {
                update('编辑用户', 'updateNotice?id=' + data.id, 700, 600);
            }
        });
