      ...
      <div style="width:100%;overflow: auto;min-height: 800px;">
        ...
        <div class="layui-form-item">
          ...
          <div class="layui-input-block">
            <input type="text" name="title" id="title" lay-verify="title" value="${informationpublish.title}" placeholder="请输入标题" autocomplete="off" class="layui-input"
                    <#if detail>
            disabled
                    </#if>>
          </div>
        ...
        <div class="layui-form-item">
          ...
          <div class="layui-inline" style="width:48%;margin: 0">
            <label class="layui-form-label" style="overflow: auto;">发布信息类型</label>
            <div class="layui-input-inline" style="width: 85%;margin: 0px 0px 0px -1px">
              ...
              disabled
                ...
                <#list informationTypes as ift>
                  <option value="${ift.code}" <#if ift.code==informationpublish.informationType>selected</#if> >${ift.dvalue}</option>
                </#list>
              </select>
            </div>
          </div>
            ...
            <div class="layui-input-inline" style="width: 85%;margin: 0px 0px 0px -1px">
              ...
              >
                <#list importantTypes as it>
                  <option value="${it.code}" <#if it.code==informationpublish.importantType>selected</#if>>${it.dvalue}</option>
                </#list>
              </select>
            </div>
          </div>
        ...
        <fieldset class="layui-elem-field layui-field-title">
          <legend style="font-size:16px;">全部附件</legend>
        </fieldset>
        ...
        <div class="layui-upload-list">
          <table class="layui-table" style="margin:0;">
            ...
            <tbody id="demoList">
            <#list informationpublish.files as file >
              <tr>
                <td>${file.fileName}</td>
                <td>${(file.fileSize/1024)?string("0.0")}KB</td>
                <td>可以下载</td>
                <td>

                  <a class="layui-btn layui-btn-xs layui-btn-info"  <#if detail>style="margin-left: 10px;"</#if> href="download?filePath=${file.filePath}&fileName=${file.fileName}">下载</a>
                  <#if !detail>
                    <a class="layui-btn layui-btn-xs layui-btn-danger" onclick="delFile('${file.filePath}','${file.id}','${informationpublish.id}',this)">删除</a>
...
<script>
  layui.use(['form','layer','upload'], function(){
    ...
    form.on('submit(add)', function(data){
      data.field.id=${informationpublish.id}
      data.field.list=JSON.stringify(mapToArray(files));
      layerAjax('updateInformationPublish',data.field, 'informationpublishList');
      return false;
    });
    form.render();
  });
