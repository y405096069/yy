...
layui.define(['jquery', 'element', 'nprogress', 'utils'], function (exports) {
    ...
    Tab.fn.tabChange = function (layId) {
        _tab.tabChange(layId)
    };
    Tab.fn.getTab = function (layId) {
        return _tab.getTab(layId);
    };
    Tab.fn.refresh = function (layId) {
        alert(layId);
        switch (_tab._config.renderType) {
            case renderType.page:
                var loadIndex = _tab.load();
                var url = _tab._title.children('li[lay-id=' + layId + ']').data('url');
                _tab._content.children('div[lay-item-id=' + layId + ']')
                    .html(_tab.getBodyContent(url + '?v=' + new Date().getTime(), function () {
                        _tab.closeLoad(loadIndex);
                    }));
                break;
            case renderType.iframe:
                var item = _tab._content.children('div[lay-item-id=' + layId + ']').children('iframe');
